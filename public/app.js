...
function renderForm(preGroup, preOffer) {
    ...
    const ageHintFn = function () {
        ...
        else if (v >= 65)       { hint.textContent = '✅ Fascia Over 65 – accedi ai percorsi per la longevità attiva!'; hint.style.cssText = 'font-size:12px;margin-top:5px;color:#1F6B52;font-weight:600;'; }
...
function renderCoupon({ name, age, email, ageGroup, gymName, gymAddress, offer, code, emailSent }) {
    const box      = document.getElementById('modal-box');
    const gColor   = ageGroup === 'Over 65' ? '#1F6B52' : '#FF6B35';
